		...
		if(P.file){
			...
			$Http[ix].onreadystatechange = function(){
				if(this.readyState === 4 && this.status === 404){
					var divnofound = $1.t("div",{'id':'psAjaxError404', style:'position:fixed; top:0; z-index:1000; left:0; width:100%; height:100%; backgroundColor:rgba(255,0,0,0.8); color:#FFF;'},document.body);
					var nofound = $1.t("div",{style:'position:absolute; margin:0 auto; height:100px; widht:100%; padding:30px 10px;'},divnofound);
					nofound.appendChild($1.t('h1',{'textNode':'Error 404'}));
					nofound.appendChild($1.t('p',{'textNode':'No se ha encontrado el recurso :'+P.file}));
					var iClose = $1.t('input',{type:'button',value:'Cerrar',style:'display:block; margin:6px 0;'},nofound);
...
$Api.form2=function(P,cont){
	AF={midCont:false,Tabs:{}};
	var pareTop=false;
	if(P.Win){
		cont=$1.t('div'); pareTop=$1.Win.open(cont,P.Win);
	}
	...
	$Api.get({furl:P.furl,f:apiGet,loadVerif:!loadVerif, inputs:vGet,loade:cont,func:function(Jr){
		...
		if(P.afterLoad){ P.afterLoad(cont, Jr, P); }
...
get:function(PARS){
	...
	if(PARS.file){
		...
		if(http.status === 404){
			...
			var nofound = $1.t("div",{style:'position:absolute; margin:0 auto; height:100px; widht:100%; padding:30px 10px;'},divnofound);
			nofound.appendChild($1.t('h1',{'textNode':'Error 404'}));
			nofound.appendChild($1.t('p',{'textNode':'No se ha encontrado el recurso :'+PARS.file}));
			var iClose = $1.t('input',{type:'button',value:'Cerrar',style:'display:block; margin:6px 0;'},nofound);
			iClose.onclick = function(){ $1.delet('psAjaxError404'); }
		}
