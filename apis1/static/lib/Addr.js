...
Addr={
	type1:function(D,pare,P){
		P=(P)?P:{};
		var jsF=(P.jsF)?P.jsF:$Api.JS.clsLN;
		var divL=$1.T.divL({divLine:1,wxn:'wrapx8',L:'Dpto.',I:{tag:'select',name:'county',opts:Addr.County,selected:D.county,'class':jsF+' AddrCounty'}},pare);
		$1.T.divL({wxn:'wrapx8',L:'Ciudad',I:{tag:'select',name:'city',opts:Addr.City,selected:D.city,'class':jsF+' AddrCity'}},divL);
		if(P.jsFk){/* agrupar por X[] */
			divL.classList.add($Api.JS.clsL);
			divL.setAttribute('jsk',P.jsFk);
		}
		if(D.addrId){ $Api.JS.addF({name:'addrId',value:D.addrId,jsF:jsF},divL); }
		var c=$1.q('.AddrCity',pare);
		c.AJs={tt:P.tt,tr:P.tr};
		Addr.dptoChan(divL);
		$1.T.divL({wxn:'wrapx2',L:'Dirección',I:{tag:'input',type:'text',name:'address',value:D.address,placeholder:'Carrera 33B','class':jsF}},divL);
		//$1.T.divL({wxn:'wrapx8',L:'Via',I:{tag:'input',type:'text',name:'streetName',value:D.streetName,placeholder:'Carrera 33B','class':jsF}},divL);
		//$1.T.divL({wxn:'wrapx10',L:'#',I:{tag:'input',type:'text',name:'num1',value:D.num1,placeholder:'29','class':jsF}},divL);
		//$1.T.divL({wxn:'wrapx10',L:'-',I:{tag:'input',type:'text',name:'guion',value:D.guion,placeholder:'10','class':jsF}},divL);
		//$1.T.divL({wxn:'wrapx8',L:'Barrio',I:{tag:'input',type:'text',name:'nBour',value:D.nBour,placeholder:'Primavera Azul','class':jsF}},divL);
		//$1.T.divL({wxn:'wrapx4',L:'Detalles',I:{tag:'input',type:'text',name:'memo',value:D.memo,placeholder:'Mz 5 Cs 9A','class':jsF}},divL);
		return divL;
	},
	draw:function(D){
		t='';
		t +=(D.address)?D.address+', ':', ';
		t +=(D.cityCode)?_g(D.cityCode,Addr.City)+', ':', ';
		t +=(D.countyCode)?_g(D.countyCode,Addr.County):'';
		return t;
	},
	basic:function(D,pare,P,topBox){ //county,city,address
		P=(P)?P:{};
		var kAddress=(P.kAddress)?P.kAddress:'address';
		var kCounty=(P.kCounty)?P.kCounty:'countyCode';
		var kCity=(P.kCity)?P.kCity:'cityCode';
		var jsF=(P.jsF)?P.jsF:$Api.JS.cls;
		var clsBox=(P.clsBox)?P.clsBox:$Api.Sea.clsBox;
		R=[];
		R[1]=$1.lTag({tag:'select',name:'countyCode',opts:Addr.County,selected:D.countyCode,'class':jsF+' AddrCounty '+clsBox,k:kCounty});
		R[1].onchange=function(){
			val =$1.qOpt(this);
			tpare=(topBox)?topBox:this.parentNode.parentNode;
			var d=$1.q('.AddrCity',tpare); 
			$1.T.sel({reLoad:d,opts:Addr.City,matchh:'^'+val});
		}
		R[2]=$1.lTag({tag:'select',name:'cityCode',opts:Addr.City,selected:D.cityCode,'class':jsF+' AddrCity '+clsBox,k:kCity});
		R[3]=$1.lTag({tag:'input',type:'text',name:kAddress,value:D[kAddress],placeholder:'Avn Ferrocaril #70-10 Bodega 501','class':jsF+' '+clsBox,k:'address'});
		if(P.nodes=='Y'){ return R; }
		var divL=$1.T.divL({divLine:1,wxn:'wrapx4',L:'Dpto.',Inode:R[1]},pare);
		$1.T.divL({wxn:'wrapx4',L:'Ciudad',Inode:R[2]},divL);
		Addr.dptoChan(divL);
		$1.T.divL({wxn:'wrapx2',L:'Dirección',Inode:R[3]},divL);
		return divL;
	},
	dptoChan:function(pare){
		var c=$1.q('.AddrCity',pare);
		var d=$1.q('.AddrCounty',pare);
		d.onchange=function(){
			val =$1.qOpt(this);
			$1.T.sel({reLoad:c,opts:Addr.City,matchh:'^'+val});
		}
	}
};
Addr.County=[
