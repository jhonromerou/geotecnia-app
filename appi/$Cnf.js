...
$Cnf.User={
	get:function(P){ P=(P)?P:{};
		var cont=$M.Ht.cont;
		$Doc.tbList({api:Api.Cnf.pr+'user',inputs:$1.G.filter(),view:'N',
		_fEdit:$Cnf.User.form,
		TD:[
			{H:'UID',k:'userId'},
			{H:'Usuario',k:'user'},
			{H:'Nombre',k:'userName'},
 ...
 form:function(P){
	 P=(P)?P:{};
	$Api.form2({uid:{k:'userId',v:P.userId},uri:Api.Cnf.pr+'user',fo:'Y',jsF:'Y',
	...
	Tabs:[
		{textNode:'Permisos',winClass:'perms',active:'Y','class':'shield',func:(T)=>{
			$Cnf.User.perms(T,T.win);
		}}
	],
	func:function(Jr,o){
			...
			var cke= ($M.uAD && $M.uAD[k]!=undefined);
				...
				if(tK.P){
					...
					for(var z in tK.P){ var ty=tK.P[z];
						...
						$1.T.ckLabel({L:{textNode:ltex},I:{'class':'jsFields',name:'P['+lk+']',checked:cke}},li0);

					}
				}
				else{
					$1.T.ckLabel({L:{textNode:tex},I:{'class':'jsFields',name:'P['+k+']',checked:cke}},L.li);
				}
...
$M.kauAssg('admin',[
	...
	{k:'admin.reports',t:'Visualizar Reportes'}
]);
$M.liAdd('cnf',[
...
{k:'cnf.user',t:'Usuarios',ini:{fNew:$Cnf.User.form,gyp:function(){ $Cnf.User.get(); } }},
{k:'cnf.ousa',t:'Control de Acceso', kau:'sysd.supersu', ini:{f:function(wFilt){
...
$M.kLiTable['cnf']={mdlActive:'cnf',MLis:['cnf.mecrd','cnf.meusr'],_F:[
	{mdlActive:'sysUsers',folId:'User',folName:'Usuarios',MLis:['cnf.user','cnf.ousp','cnf.ousa','cnf.repAssg']}
]};
