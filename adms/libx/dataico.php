...
static $uri='https://api.dataico.com/direct/dataico_api/v2/';
...
static public function response($R=array()){
	...
	if($R['errNo'] && $R['text']){ $R['errors']=array(['error'=>$R['text']]); }
...
static public function convFie($P=array()){
	...
	{$X['invoice']=array(
		...
		*/
		...
		'number'=>$P['number'],
		'numbering'=>[
			'resolution_number'=> self::$D['invoice_resolution_number'],
			'prefix'=> self::$D['invoice_prefix'],
			//'number'=> $P['number'],
			'flexible'=> self::$D['invoice_flexible'],
		],
		'customer'=>array(
			'email'=>$P['email'],
			'phone'=>$P['phone1'],
...
static public function invPost($P=array()){
	...
	unset($RteB);
	$X['invoice']['items']=$Li;
	$R=xCurl::post(self::$uri.'invoices',
		array('__JSON'=>$X),
		array('h'=>array('auth-token: '.self::$D['token']), 'utf8Plain'=>'Y'));
	if(xCurl::$errno){
		return _err::err(xCurl::$errno_text,3);
	}
	$R=self::response($R);
	if(self::$err){ _err::err('[libx] Errores en generación con proveedor dataico: '.self::$errText.$ori, 3); }
