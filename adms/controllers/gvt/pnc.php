...
static $serie='gvtPnc';
static $tbk='gvt_opnc';
static $tbk1='gvt_pnc1';
static $tbk2='gvt_pnc2';
...
static public function getOne($D){
	_ADMS::lib('iDoc');
	$D['fromA']='C.cardCode, C.licTradType,C.licTradNum,A.*
	FROM gvt_opnc A
	LEFT JOIN par_ocrd C ON (C.cardId=A.cardId)';
	$D['fromB']='I.itemCode, I.itemName,I.buyUdm, B.* FROM gvt_pnc1 B
...
static public function post($_J=array()){
	...
	if(_err::$err){ return _err::$errText; }
		...
		foreach($_J['L'] as $nx=>$L){
			...
			if(_err::$err){ return _err::$errText; }
			$costT=$L['priceLine'];
			if(!$nDoc->fromDlv && $L['handInv']=='Y'){
				$L['outQty']=$L['quantity'];
				$nDoc->handSet($L); unset($L['outQty']);
...
static public function dacPost($nDoc){
	...
	$nDac->setDocTax(array('buyIva'=>'cre')); if(_err::$err){ return false; }
	$nDac->setLine([
	array('lineType'=>'NC','dueDate'=>$nDac->Doc['docDate'],'cardId'=>$nDac->Doc['cardId'],'accId'=>$nDac->Doc['accId'],'accCode'=>'22/11xx','debBal'=>$nDac->Doc['docTotal'],'debBalDue'=>$nDac->Doc['docTotal'])
	]);
