...
if(_0s::$router=='GET cph'){
	_ADMS::lib('iDoc');
	$_GET['fromA']='A.* FROM ivt_ocph A ';
	echo iDoc::get($_GET);
}

else if(_0s::$router=='GET cph/view'){
	$He= Doc::view(array('docEntry'=>$_GET['docEntry'],'sel_otb'=>'docStatus,docDate,whsId,lineMemo FROM ivt_ocph'));
	if(!is_array($He)){ echo $He; }
	else{
		$ql=a_sql::query('SELECT B.tt,B.tr,B.lineNum,B.itemIdFrom,B.itemSzIdFrom, B.itemIdTo,B.itemSzIdTo,B.quantity FROM ivt_cph1 B WHERE docEntry=\''.$_GET['docEntry'].'\' ',array(1=>'Error obteniendo lineas del documento: '));
...
else if(_0s::$router=='PUT cph/close'){
	$D=$___D; unset($___D);
	_ADMS::lib('iDoc');
	if(iDoc::vStatus(array('tbk'=>'ivt_ocph','docEntry'=>$D['docEntry']))){ die(_err::$errText); }
	else{
		$q=a_sql::query('SELECT A.whsId,B.itemIdFrom,B.itemSzIdFrom, B.itemIdTo,B.itemSzIdTo,B.quantity FROM ivt_cph1 B JOIN ivt_ocph A ON (A.docEntry=B.docEntry) WHERE A.docEntry=\''.$D['docEntry'].'\' ',array(1=>'Error obteniendo artículos para cambio:',2=>'El documento no tiene lineas guardadas.'));
		if(a_sql::$err){ $js=a_sql::$errNoText; }
		else{
			$Ii=array(); $Liv=array();
			$docDate=date('Y-m-d');
			...
			Ivt::onHand_put($Liv,array('docDate'=>date('Y-m-d'),'tt'=>$serieType,'tr'=>$D['docEntry']));
			if(_err::$err){ $js= _err::$errText; }
			else{
				a_sql::query('UPDATE ivt_ocph SET docStatus=\'C\', invMov=\'Y\' WHERE docEntry=\''.$D['docEntry'].'\' LIMIT 1',array(1=>'Error actualizando estado del documento: '));
				if(a_sql::$err){ $js=a_sql::$errNoText; }
				else{ $commit=true; $js=_js::r('Documento Actualizado correctamente.'); }
			}
